...
function renderCart(focus){
 ...
 ${r.method==='store'?`<label>受け取り場所<select class="${r.pickup?'':'pickup-unselected'}" aria-required="true" data-field="pickup" aria-label="明細${i+1}の受け取り場所">${options(p.pickup,r.pickup)}</select></label>`:r.method==='delivery'?'<p class="delivery-info">ご入力の住所へ配達します。<br>配達料：申込施設ごとに1,000円</p>':p.delivery?'<p class="delivery-info pickup-unselected-text">受け取り方法を選択してください。</p>':''}
...
productGrid.addEventListener('click',e=>{
 ...
 if(existing)existing.qty+=qty;else cart.push({id:++serial,no:p.no,qty,method:p.delivery?'':'store',pickup:'',date:''});
...
cartItems.addEventListener('click',e=>{
 ...
 if(b.dataset.copy){const r=cart.find(r=>r.id===Number(b.dataset.copy));const id=++serial;const p=byNo(r.no);cart.push({id,no:r.no,qty:1,method:p.delivery?'':'store',pickup:'',date:''});invalidate();renderCart({id,field:'qty'});cartItems.querySelector(`[data-row="${id}"]`).scrollIntoView({behavior:'smooth',block:'center'});toast('別の受け取り分を1点追加しました。場所と日時を指定してください。');}
...
form.addEventListener('submit',e=>{
 ...
 const incomplete=cart.find(r=>!r.date||(r.method==='store'&&!r.pickup)||(byNo(r.no).delivery&&!r.method));
 if(incomplete){setMsg('各商品の受け取り方法・場所・日時を選択してください。','err');const field=byNo(incomplete.no).delivery&&!incomplete.method?'method':incomplete.method==='store'&&!incomplete.pickup?'pickup':'date';const el=cartItems.querySelector(`[data-row="${incomplete.id}"] [data-field="${field}"]`);el.focus();return;}
