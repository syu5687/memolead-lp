...
fsOptions?.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{document.body.classList.remove('fs-1','fs-2');if(Number(b.dataset.fs))document.body.classList.add('fs-'+b.dataset.fs);fsOptions.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===b));fsCtrl.classList.remove('open');fsToggle?.setAttribute('aria-expanded','false');if(fsOptions)fsOptions.hidden=true;}));
const cartDock=document.querySelector('.cart-dock'),cartSection=document.getElementById('cart');
if(cartDock&&cartSection&&'IntersectionObserver' in window){new IntersectionObserver(([entry])=>cartDock.classList.toggle('at-cart',entry.isIntersecting),{threshold:0.01}).observe(cartSection);}
...
form.addEventListener('submit',async e=>{
 e.preventDefault();invalidate();if(!cart.length){setMsg('ご注文の商品を1点以上お選びください。','err');return;}
 ...
 if(incomplete){setMsg('各商品の受け取り方法・場所・日時を選択してください。','err');const field=byNo(incomplete.no).delivery&&!incomplete.method?'method':incomplete.method==='delivery'&&!incomplete.time?'time':incomplete.method==='store'&&!incomplete.pickup?'pickup':'date';cartItems.querySelector(`[data-row="${incomplete.id}"] [data-field="${field}"]`)?.focus();return;}
 ...
 const t=totals(),orders=[];Object.entries(FACILITIES).forEach(([id,f])=>{const rows=cart.filter(r=>byNo(r.no).facilityId===id);if(!rows.length)return;let subtotal=0,subtax=0;const items=rows.map(r=>{const p=byNo(r.no),line=unit(p)*r.qty,lineTax=tax(p)*r.qty;subtotal+=line;subtax+=lineTax;return {no:p.no,name:p.name,qty:r.qty,unit:unit(p),line,tax:tax(p),lineTax};});const delivery=rows.some(r=>r.method==='delivery');orders.push({facilityId:id,facility:f.name,category:rows.map(r=>byNo(r.no).category).filter((v,i,a)=>a.indexOf(v)===i).join('・'),items,method:delivery?'福岡県内配達':'店頭受け取り',pickup:delivery?'（ご住所へ配達）':rows.find(r=>r.pickup)?.pickup||'',pickdate:rows.map(r=>r.date).filter((v,i,a)=>a.indexOf(v)===i).join('／'),times:rows.map(r=>r.time).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join('／'),subtotal,subtax,fee:delivery?1000:0});});
 const data={name:fd.get('name').trim(),zip:fd.get('zip').trim(),address:fd.get('address').trim(),tel:fd.get('tel').trim(),email:fd.get('email').trim(),note:fd.get('note')?.trim()||'',tier,tierLabel:tier==='special'?'特別価格（メモリード会員）':'一般価格',facility:orders.map(o=>o.facility).join('・'),orders,total:t.total,totalTax:t.productTax,summary:''};
 submitBtn.disabled=true;submitBtn.textContent='送信中…';try{const r=await fetch('https://memolead-fukuoka-osechi.mk-cbe.workers.dev/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});if(!r.ok)throw Error('status '+r.status);form.style.display='none';document.getElementById('testSummary').textContent='申込内容をメールでお送りしました。';review.hidden=false;review.scrollIntoView({behavior:'smooth',block:'start'});sessionStorage.removeItem(storageKey);}catch(err){setMsg('送信に失敗しました。時間をおいて再度お試しください。','err');submitBtn.disabled=false;submitBtn.textContent='この内容で申し込む';}}
);
renderCart();
