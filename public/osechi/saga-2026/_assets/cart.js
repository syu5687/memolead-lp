...
form.addEventListener('submit',async e=>{
 e.preventDefault();invalidate();if(!SAGA_ORDER_READY){setMsg('受付準備中です。お問い合わせは0952-23-0111へお願いいたします。','err');return;}if(!cart.length){setMsg('ご注文の商品を1点以上お選びください。','err');return;}
 if(!tier){setMsg('価格区分を選択してください。','err');document.querySelector('[name="ptier"]').focus();return;}
 const incomplete=cart.find(r=>(byNo(r.no).cakes&&!r.cake)||!r.date||(r.method==='store'&&!r.pickup)||(byNo(r.no).delivery&&(!r.method||(r.method==='delivery'&&!r.time))));
 if(incomplete){setMsg('各商品の受け取り方法・場所・日時を選択してください。','err');const field=byNo(incomplete.no).cakes&&!incomplete.cake?'cake':byNo(incomplete.no).delivery&&!incomplete.method?'method':incomplete.method==='delivery'&&!incomplete.time?'time':incomplete.method==='store'&&!incomplete.pickup?'pickup':'date';cartItems.querySelector(`[data-row="${incomplete.id}"] [data-field="${field}"]`)?.focus();return;}
