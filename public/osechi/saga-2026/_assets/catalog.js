// @version v0005 | 2026-09-16 | 佐賀 商品カタログ
...
catalogProducts.forEach(p=>{
  ...
  const prices=isEarlyOrderPeriod()&&p.e!=null
    ? `<div class="early-price"><span>早期購入価格（一般・会員共通）</span><strong>${p.e.toLocaleString('ja-JP')}<small>円</small></strong></div><div><span>通常価格</span><strong>${p.g.toLocaleString('ja-JP')}<small>円</small></strong></div><div><span>会員価格</span><strong>${p.s.toLocaleString('ja-JP')}<small>円</small></strong></div>`
    : p.price!=null
