...
var CONFIG = {
  ...
  VENUE: { name: "サロン・ド・ルシェル", address: "佐賀県佐賀市多布施2丁目15-1", tel: "0952-20-1516", telNote: "木曜日店休" },                       // 申込者への受付確認メール
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      const venueHtml = `<div style="margin-top:18px;padding:14px;border:1px solid #d8cdb9;border-radius:8px;background:#f6f2ea;font-size:14px;line-height:1.8;">
        ...
        <div>TEL <a href="tel:${esc(V.tel.replace(/-/g, ""))}" style="color:#7c1f2a;">${esc(V.tel)}</a>${V.telNote ? `（${esc(V.telNote)}）` : ""}</div>
      </div>`;
      ...
      if (CONFIG.AUTO_REPLY && emailOk) {
        const custHtml = `
          <div style="font-family:sans-serif;max-width:640px;margin:0 auto;padding:20px;color:#222;line-height:1.8;">
            <p>${esc(d.name || "")} 様</p>
            <p>この度は指輪＆結婚相談会にお申し込みいただき、ありがとうございます。<br>以下の内容で承りました。ご希望の日時につきましては、担当者より改めてご連絡いたします。</p>
